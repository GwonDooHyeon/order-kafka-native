...
graph TD
    A["Producer"]
    A -->|" Key: order-001 "| B["hash 계산<br/>10 % 3 = 0"]
    A -->|" Key: order-001 "| C["hash 계산<br/>10 % 3 = 0"]
    A -->|" Key: order-002 "| D["hash 계산<br/>7 % 3 = 1"]
    A -->|" Key: order-003 "| E["hash 계산<br/>5 % 3 = 2"]
    B --> F["Partition 0<br/>(order-001의 모든 메시지)"]
    C --> F
    D --> G["Partition 1<br/>(order-002의 메시지)"]
    E --> H["Partition 2<br/>(order-003의 메시지)"]
    style F fill: #ffffcc
    style G fill: #ffffcc
    style H fill: #ffffcc
```
